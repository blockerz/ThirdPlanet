...
public class AStarPathFinder1 {
	...
	public ArrayList<Point> findPath(Entity entity, Point start, Point end,
			int maxTries) {
		open.clear();
		closed.clear();
		parents.clear();
	...
	private void checkNeighbors(Entity entity, Point end, Point closest) {
		for (Point neighbor : closest.neighbors()) {
			if (closed.contains(neighbor)
					|| !entity.canEnter(neighbor.x(), neighbor.y())
					&& !neighbor.equals(end))
				continue;
